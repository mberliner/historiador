...
class TestExecuteMethod:
    ...
    def test_execute_successful_diagnosis(self, mock_settings, mock_jira_client):
        ...
        assert result['config_suggestion'] is not None
        parsed_config = json.loads(result['config_suggestion'])
        assert parsed_config == result['required_fields']
        
        # Verify current config
        assert result['current_config']['feature_type'] == "Feature"
        assert result['current_config']['required_fields'] == {"summary": "Summary"}
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_with_project_key_override(self, mock_settings, mock_jira_client):
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_with_no_required_fields(self, mock_settings, mock_jira_client):
        ...
        mock_feature_manager = Mock()
        mock_feature_manager.get_required_fields_for_feature.return_value = None
        ...
        # Verify result when no required fields
        assert result['required_fields'] is None
        assert result['config_suggestion'] is None

    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_with_empty_required_fields(self, mock_settings, mock_jira_client):
        ...
        assert result['required_fields'] == {}
        assert result['config_suggestion'] is None  # Empty dict is falsy, so None
    ...
    def test_execute_result_structure(self, mock_settings, mock_jira_client):
        ...
        assert 'config_suggestion' in result
        assert 'current_config' in result
        assert len(result) == 5  # Should have exactly these 5 keys
        
        # Verify current_config structure
        assert isinstance(result['current_config'], dict)
        assert 'feature_type' in result['current_config']
        assert 'required_fields' in result['current_config']
        assert len(result['current_config']) == 2
...
class TestErrorHandling:
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_connection_failure(self, mock_settings, mock_jira_client):
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_project_not_found(self, mock_settings, mock_jira_client):
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_invalid_feature_type(self, mock_settings, mock_jira_client):
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_feature_manager_error(self, mock_settings, mock_jira_client):
...
class TestEdgeCases:
    ...
    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_with_complex_required_fields(self, mock_settings, mock_jira_client):
        ...
        assert result['required_fields'] == complex_fields
    ...
    def test_execute_multiple_calls_independence(self, mock_settings, mock_jira_client):
        ...
        assert result1['required_fields'] == {"req1": "val1"}
        assert result1['current_config']['required_fields'] == {"field1": "value1"}
        
        # Second call
        result2 = use_case.execute()
        assert result2['project_key'] == "PROJ2"
        assert result2['feature_type'] == "Epic"
        assert result2['required_fields'] == {"req2": "val2"}
        assert result2['current_config']['required_fields'] == {"field2": "value2"}

    @patch('src.application.use_cases.diagnose_features.JiraClient')
    @patch('src.application.use_cases.diagnose_features.Settings')
    def test_execute_with_none_project_key(self, mock_settings, mock_jira_client):
